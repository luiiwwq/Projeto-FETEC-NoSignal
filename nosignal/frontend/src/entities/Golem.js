 ...
 * range. Five of them are spawned as a wave every third night (see
...
// 2 south (bottom), 1 north (top), 2 west (front, near the player's landing
// zone/cave entrance). All five sit on open ground away from the obstacles,
// the player and each other. Blocked spots fall back to the nearest free tile.
export const GOLEM_WAVE_SPAWNS = {
    bottom: [{ x: 812, y: 1484 }, { x: 900, y: 1484 }],
    top: [{ x: 1000, y: 250 }],
    front: [{ x: 900, y: 350 }, { x: 800, y: 1150 }]
};
...
export class Golem {
    ...
    _moveDir(nx, ny, dt) {
        this.vx = nx * GOLEM_SPEED;
        this.vy = ny * GOLEM_SPEED;
        ...
        if (
            ...
            if (target && !target.isDead && typeof target.takeDamage === 'function') {
                ...
                if (Math.hypot(target.x - this.x, target.y - this.y) <= reach) {
                    target.takeDamage(GOLEM_ATTACK_DAMAGE, this.x, this.y);
                }
            }
        }
